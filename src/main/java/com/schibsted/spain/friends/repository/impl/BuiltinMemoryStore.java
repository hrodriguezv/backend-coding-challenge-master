...
public class BuiltinMemoryStore {
    ...
    public User addUser(String key, User value) {
        ...
        if (!violations.isEmpty()) manageBaseException(violations.iterator().next().getMessage());

        if (instance.users.get(key) != null) throw new ConstraintViolationException("unique constraint :" + key, Collections.emptySet());
        
        instance.users.put(key, value);
    ...
    public Friendship addFriendship(FriendshipPK key, Friendship value) {
        return instance.relations.putIfAbsent(key, value);
    }
